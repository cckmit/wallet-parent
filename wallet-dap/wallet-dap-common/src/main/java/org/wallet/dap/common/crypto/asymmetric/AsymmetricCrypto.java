package org.wallet.dap.common.crypto.asymmetric;
...
import org.wallet.dap.common.crypto.symmetric.SymmetricAlgorithm;
...
public class AsymmetricCrypto implements Crypto {
	...
	/**
	 * 构造，创建新的私钥公钥对
	 * @param algorithm {@link org.wallet.dap.common.crypto.symmetric.SymmetricCrypto}
	 */
	...
	public byte[] encrypt(byte[] data, KeyType keyType) {
		lock.lock();
		try {
			cipher.init(Cipher.ENCRYPT_MODE, getKeyByType(keyType));
	...
	public byte[] decrypt(byte[] bytes, KeyType keyType) {
		lock.lock();
		try {
			cipher.init(Cipher.DECRYPT_MODE, getKeyByType(keyType));
